    ...
    class Sokoban
    {
        static void Main()
        {
            ...
            Box[] boxes = new Box[]
            {
                ...
                new Box { X = 15, Y = 8, IsOnGoal = false},
                new Box { X = 22, Y = 6, IsOnGoal = false}
            };
            ...
            void Render()
            {
                ...
                {
                    RenderObject(goals[goalId].X, goals[goalId].Y, "G");
                }
                ...
                // 폭탄을 그린다.
                for (int bombId = 0; bombId < BOMB_COUNT; ++bombId)
                {
                    RenderObject(bombs[bombId].X, bombs[bombId].Y, "B");
                }
            ...
            void Update(ConsoleKey key)
            {
                ...
                {
                    if (false == IsCollided(player.X, player.Y, bombs[i].X, bombs[i].Y))
                    {
                        continue;
                    }
                    switch (player.MoveDirection)
